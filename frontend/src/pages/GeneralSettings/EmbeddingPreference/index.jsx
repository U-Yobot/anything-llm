...
export default function GeneralEmbeddingPreference() {
  ...
  const selectedEmbedderObject = EMBEDDERS.find(
    ...
    value: selectedEmbedder || "native",
    logo: AnythingLLMIcon,
    description: "Loading embedder configuration...",
    options: () => null,
  };

  return (
    <div className="flex w-screen h-screen overflow-hidden bg-theme-bg-container">
      <Sidebar />
      {loading ? (
        <div
        ...
        >
          ...
          >
            <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] py-16 md:py-6">
              <div className="flex flex-col w-full pb-6 border-b-2 border-white gap-y-1 light:border-theme-sidebar-border border-opacity-10">
              ...
              <div className="flex justify-end w-full">
                {hasChanges && (
                  <CTAButton
                    ...
                    className="z-10 mt-3 mr-0 -mb-14"
                  >
                    {saving ? t("common.saving") : t("common.save")}
                  </CTAButton>
                )}
              </div>
              <div className="mt-6 mb-4 text-base font-bold text-white">
                {t("embedding.provider.title")}
              </div>
              <div className="relative">
                {searchMenuOpen && (
                  <div
                    className="fixed top-0 left-0 z-10 w-full h-full bg-black bg-opacity-70 backdrop-blur-sm"
                    ...
                    <div className="flex flex-col w-full gap-y-1">
                      <div className="flex items-center sticky top-0 z-10 border-b border-[#9CA3AF] mx-4 bg-theme-settings-input-bg">
                        <MagnifyingGlass
                          size={20}
                          weight="bold"
                          className="absolute z-30 my-2 -ml-4 left-4 text-theme-text-primary"
                        />
                        <input
                          type="text"
                        ...
                        <X
                          size={20}
                          weight="bold"
                          className="text-white cursor-pointer hover:text-x-button"
              ...
              >
                {selectedEmbedder && selectedEmbedderObject?.options(settings)}
              </div>
            </div>
          </form>
        </div>
